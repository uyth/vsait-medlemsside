$(document).ready(() => {
	...
	for (save of saves) {
		...
		$(input).click(function() {
			...
			const startTime = new Date(startTime_date.value + " " + startTime_time.value);
			const endTime = new Date(endTime_date.value + " " + endTime_time.value);
			const registrationDeadline = new Date(registrationDeadline_date.value + " " + registrationDeadline_time.value);
			const cancellationDeadline = new Date(cancellationDeadline_date.value + " " + cancellationDeadline_time.value);
			// If timedate is valid, check for time validity given: påmeldingsfrist <= avmeldingsfrist <= starttid < sluttid
			// else proceed as normal
			if (isValid(startTime) && isValid(endTime) && isValid(registrationDeadline) && isValid(cancellationDeadline)) {
				console.log("All valid");
				if (registrationDeadline <= cancellationDeadline && cancellationDeadline <= startTime && startTime < endTime) {
					console.log("Saving..");
					save.click();
				} else {
					console.log("Error..");
