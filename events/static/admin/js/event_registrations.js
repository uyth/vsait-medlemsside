$(document).ready(() => {
	...
	const createTable = (name_array,header,div_parent,div_location,hidden) => {
	...
	const createTables = function() {
		...
		createTable(registered_names, header, div_registrations_parent,div_registrations_parent, false);
		createTable(names, header, div_registrations_parent,div_registrations_parent, true);
		createTable(waiting_list_names, header, div_waiting_list_parent,div_waiting_list_parent, false);
		createTable(names, header, div_waiting_list_parent,div_waiting_list_parent, true);
	}
	createTables();
	...
	const event_id = +window.location.pathname.split("/").filter(x => x.length > 0 && x.length <= 4)[0]
	$(".field-secret_url").get(0).innerHTML += `<div><a href='/events/${event_id}/${secret_url}' target='_blank' >Open checkin form</div>`
});
